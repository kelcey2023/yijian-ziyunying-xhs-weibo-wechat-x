...
这套仓库的底层执行框架是 `OpenClaw + CDP + 浏览器自动化脚本`。
如果你当前是通过 `Hermes` 来调用、协作或生成部分内容，那么 `Hermes` 更接近“运行入口 / 协作界面 / 内容生成补充层”，而不是这套工程的底层自动化框架。

换句话说：

> 这是一个基于 `OpenClaw` 的四平台自动运营工程，可以通过 `Hermes` 来驱动和扩展，但仓库本身不是纯 `Hermes` 工程。
